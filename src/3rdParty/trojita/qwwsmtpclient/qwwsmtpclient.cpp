    ...
    while (socket->canReadLine()) {
        ...
        if (mid){
            ...
            case SMTPCommand::Authenticate: {
                int stage = cmd.extra.toInt();
                if (stage == 1 && status == 535) {
                    // unrecognized username or password
                    errorString.append(rx.cap(2).trimmed() + "\n");
                }
            }
            default: break;
            }
            ...
            if (last) {
                ...
                case SMTPCommand::Authenticate: {
                    ...
                    if (stage==0 && status==334) {
                        ...
                        errorString.append(rxlast.cap(2).trimmed());
                        setState(QwwSmtpClient::Connected);
                        emit q->done(false);
                    }
                }
...
void QwwSmtpClientPrivate::processNextCommand(bool ok) {
    ...
    case SMTPCommand::Authenticate: {
        ...
        if (authmode == QwwSmtpClient::AuthAny){
            bool modified = false;
            if (authModes.testFlag(QwwSmtpClient::AuthLogin)) {
                authmode = QwwSmtpClient::AuthLogin;
                modified = true;
            } else if (authModes.testFlag(QwwSmtpClient::AuthPlain)) {
        ...
        case QwwSmtpClient::AuthLogin:
            ...
            break;
        default:
            errorString = QString("Unsupported or unknown authentication scheme");
            emit q->done(false);
...
void QwwSmtpClientPrivate::sendEhlo() {
    SMTPCommand &cmd = commandqueue.head();
    QString domain;
    if (localName == "localhost" || localName.isEmpty()) {
        if (socket->localAddress().toString() != NULL)
            domain = socket->localAddress().toString();
    };
