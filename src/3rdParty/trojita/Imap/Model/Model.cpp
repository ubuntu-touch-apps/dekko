                ...
                if (! handled) {
                    ...
                    try {
                        logTrace(it->parser->parserId(), Common::LOG_TASKS, QString(),
                                 QString::fromLatin1("Routing to %1 %2").arg((*taskIt)->metaObject()->className(),
...
void Model::askForPreview(TreeItemMessage *item, Model::PreloadingMode preloadMode)
{
    ...
    if (item->uid()) {
        item->data()->m_preview = cache()->messagePreview(mailboxPtr->mailbox(), item->uid());
        if (!item->data()->m_preview.isEmpty())
        {
            item->setPreviewFetchStatus(TreeItem::DONE);
        }
    }

    if (networkPolicy() == NETWORK_OFFLINE) {
        if (item->previewFetchState() != TreeItem::DONE)
            item->setPreviewFetchStatus(TreeItem::UNAVAILABLE);
    } else if (item->previewFetchState() != TreeItem::DONE) {
        // request Preview Download if can find a main text part in the message
        QModelIndex messageIndex = item->toIndex(this);
        QModelIndex idx;
        QString partMessage;
        Imap::Mailbox::FindInterestingPart::MainPartReturnCode res =
        ...
        case Imap::Mailbox::FindInterestingPart::MAINPART_PART_LOADING:
        {
            ...
            TreeItemPart * part = dynamic_cast<TreeItemPart *>(realTreeItem(idx, 0, &idx));
            findTaskResponsibleFor(mailboxPtr)->requestPreviewDownload(item->uid(), part->partIdForFetchPreview());
            item->setPreviewFetchStatus(TreeItem::LOADING);
            break;
        }
        case Imap::Mailbox::FindInterestingPart::MAINPART_MESSAGE_NOT_LOADED:
            // should wait for message body structure to load
            break;
        case Imap::Mailbox::FindInterestingPart::MAINPART_PART_CANNOT_DETERMINE:
            // message doesn't have a plain text part for preview, give up.
            item->setPreviewFetchStatus(TreeItem::UNAVAILABLE);
            break;
        }
    }
    if (networkPolicy() == NETWORK_ONLINE && preloadMode == PRELOAD_PER_POLICY) {
        int preload = 20;
        int order = item->row();
        for (int i = qMax(0, order - preload); i < qMin(list->m_children.size(), order + preload); ++i) {
            ...
            Q_ASSERT(message);
            if (item != message && !message->previewFetched() && !message->previewLoading() && message->uid()) {
                askForPreview(message, PRELOAD_DISABLED);
            }
        }
    }
...
QByteArray Model::oauthAccessToken() const
{
    return m_oauthAccessToken;
}

void Model::setOauthAccessToken(const QByteArray &token)
{
    m_oauthAccessToken = token;
...
TreeItemMailbox *Model::getToplevelMailboxItem()
{
    return m_mailboxes;
}
