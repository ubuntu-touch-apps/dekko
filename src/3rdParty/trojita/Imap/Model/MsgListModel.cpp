...
MsgListModel::MsgListModel(QObject *parent, Model *model): QAbstractProxyModel(parent), msgListPtr(0), waitingForMessages(false)
{
    ...
    connect(model, SIGNAL(messageCountPossiblyChanged(QModelIndex)), this, SLOT(handleMessageCountPossiblyChanged(QModelIndex)));
...
#endif
{
    ...
    if (roleNames.isEmpty()) {
        ...
        roleNames[RoleMessagePreview] = "preview";
    }
    return roleNames;
}
...
void MsgListModel::handleMessageCountPossiblyChanged(QModelIndex mailboxIndex)
{
    checkPersistentIndex();
    if (! msgListPtr)
        return;
    TreeItem *item = Model::realTreeItem(mailboxIndex);
    TreeItemMailbox *mailbox = dynamic_cast<TreeItemMailbox *>(item);
    Q_ASSERT(mailbox);
    if (mailbox == msgListPtr->parent()) {
        emit unreadCountPossiblyChanged();
    }
...
QString MsgListModel::mailboxName() const
{
    QModelIndex idx = msgList.parent();
    if (!idx.isValid()) {
        return QString();
    }
    return idx.data(Imap::Mailbox::RoleMailboxName).toString();
}
...
int MsgListModel::unreadCount() const
{
    QModelIndex idx = msgList.parent();
    if (!idx.isValid()) {
        return 0;
    }
    return idx.data(Imap::Mailbox::RoleUnreadMessageCount).toInt();
}
