...
#endif
{
    ...
    if (roleNames.isEmpty()) {
        ...
        roleNames[RoleMailboxSpecialUseGmailIsIMPORTANT] = "isGmailImportantSpecialUse";
        roleNames[RoleMailboxSpecialUseIsALL] = "mailboxIsSpecialUseALL";
        roleNames[RoleMailboxSpecialUseIsARCHIVE] = "mailboxIsSpecialUseARCHIVE";
        roleNames[RoleMailboxIsDRAFTS] = "mailboxIsDRAFTS";
        roleNames[RoleMailboxSpecialUseIsDRAFTS] = "mailboxIsSpecialUseDRAFTS";
        roleNames[RoleMailboxSpecialUseIsFLAGGED] = "mailboxIsSpecialUseFLAGGED";
        roleNames[RoleMailboxIsJUNK] = "mailboxIsJUNK";
        roleNames[RoleMailboxSpecialUseIsJUNK] = "mailboxIsSpecialUseJUNK";
        roleNames[RoleMailboxIsSENT] = "mailboxIsSENT";
        roleNames[RoleMailboxSpecialUseIsSENT] = "mailboxIsSpecialUseSENT";
        roleNames[RoleMailboxIsTRASH] = "mailboxIsTRASH";
        roleNames[RoleMailboxSpecialUseIsTRASH] = "mailboxIsSpecialUseTRASH";
        roleNames[RoleAccountId] = "accountId";
        roleNames[RoleMailboxNestingDepth] = "nestingDepth";
    }
    return roleNames;
}
...
QVariant MailboxModel::data(const QModelIndex &proxyIndex, int role) const
{
    ...
    if (role > RoleBase && role < RoleInvalidLastOne)
        if (role == RoleAccountId) {
            const QObject *t = this;
            Dekko::Accounts::Account *acc = qobject_cast<Dekko::Accounts::Account *>(t->parent());
            Q_ASSERT(acc);
            return acc->accountId();
        } else if (role == RoleMailboxNestingDepth) {
            // Here we basically get the full mailbox name then split it at it's seperator
            // and return it's size.
            QString m = proxyIndex.data(Imap::Mailbox::RoleMailboxName).toString();
            QStringList ml = m.split(proxyIndex.data(Imap::Mailbox::RoleMailboxSeparator).toString());
            return ml.size();
        } else {
