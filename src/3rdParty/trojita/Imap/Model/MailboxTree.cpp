...
QVariant TreeItemMailbox::data(Model *const model, int role)
{
    ...
    case RoleMailboxSpecialUseIsALL:
        return QVariant::fromValue<bool>(m_metadata.flags.contains(QLatin1String("\\ALL")));
    case RoleMailboxSpecialUseIsARCHIVE:
        return QVariant::fromValue<bool>(m_metadata.flags.contains(QLatin1String("\\ARCHIVE")));
    case RoleMailboxSpecialUseIsDRAFTS:
        return QVariant::fromValue<bool>(m_metadata.flags.contains(QLatin1String("\\DRAFTS")));
    case RoleMailboxSpecialUseIsFLAGGED:
        return QVariant::fromValue<bool>(m_metadata.flags.contains(QLatin1String("\\FLAGGED")));
    case RoleMailboxSpecialUseIsJUNK:
        return QVariant::fromValue<bool>(m_metadata.flags.contains(QLatin1String("\\JUNK")));
    case RoleMailboxSpecialUseIsSENT:
        return QVariant::fromValue<bool>(m_metadata.flags.contains(QLatin1String("\\SENT")));
    case RoleMailboxSpecialUseIsTRASH:
        return QVariant::fromValue<bool>(m_metadata.flags.contains(QLatin1String("\\TRASH")));
    case RoleMailboxSpecialUseGmailIsIMPORTANT:
        return QVariant::fromValue<bool>(m_metadata.flags.contains(QLatin1String("\\IMPORTANT")));
    case RoleMailboxIsDRAFTS:
        return mailbox().toUpper().contains(QLatin1String("DRAFTS"));
    case RoleMailboxIsSENT:
        return mailbox().toUpper().contains(QLatin1String("SENT")) ||
               mailbox().toUpper().contains(QLatin1String("SENT MAIL"));
    case RoleMailboxIsJUNK:
        return mailbox().toUpper().contains(QLatin1String("JUNK")) ||
               mailbox().toUpper().contains(QLatin1String("SPAM"));
    case RoleMailboxIsTRASH:
        return mailbox().toUpper().contains(QLatin1String("TRASH"));
    default:
        return QVariant();
    }
}
        ...
        TreeItemMessage *&changedMessage, bool usingQresync)
{
    ...
    for (Responses::Fetch::dataType::const_iterator it = response.data.begin(); it != response.data.end(); ++ it) {
        if (it.key() == "UID") {
            ...
            changedMessage = message;
        } else if (it.key().startsWith("BODY[") && it.key().endsWith(">")) {
            // get fetch preview response
            const QByteArray &data = static_cast<const Responses::RespData<QByteArray>&>(*(it.value())).data;
            TreeItemPart *part = partIdToPtr(model, message, it.key().mid(0, it.key().indexOf("<0>")));
            QByteArray decodedData;
            Imap::decodeContentTransferEncoding(data, part->encoding(), &decodedData);
            message->data()->m_preview = removeEmptyLine(decodeByteArray(decodedData, part->charset()));
            if (message->uid()) {
                model->cache()->setMessagePreview(mailbox(), message->uid(), message->data()->m_preview);
            }
            changedMessage = message;
            message->setPreviewFetchStatus(DONE);
...
QString TreeItemMailbox::removeEmptyLine(QString string)
{
...
void TreeItemMsgList::fetch(Model *const model)
{
    ...
    if (!loading() && toIndex(model).isValid()) {
        setFetchStatus(LOADING);
...
TreeItemMessage::TreeItemMessage(TreeItem *parent):
    TreeItem(parent), m_offset(-1), m_uid(0), m_data(0), m_flagsHandled(false), m_wasUnread(false), m_previewFetchState(NONE)
{
}
...
QVariant TreeItemMessage::data(Model *const model, int role)
{
    ...
    case RoleMessageFuzzyDate:
    {
        ...
        QDateTime timestamp = envelope(model).date.toLocalTime();
        ...
        if (beforeDays == 1) {
            return Model::tr("Yesterday");
        }
        if (beforeDays <= 7) {
            return QDate(timestamp.date().year(), timestamp.date().month(), timestamp.date().day()).toString(Model::tr("dddd"));
        }

        //: The format specifiers (yyyy, etc) must not be translated, but
        //: their order can be changed to follow the local conventions
        return QDate(timestamp.date().year(), timestamp.date().month(), timestamp.date().day()).toString(Model::tr("dd MMMM yyyy"));
    }
...
QString TreeItemMessage::preview()
{
    return data()->m_preview;
}

void TreeItemMessage::fetchPreview(Model *model)
{
    if (previewFetched() || previewLoading() || isUnavailable(model))
        return;

    if (m_uid) {
        // find preview text part
        model->askForPreview(this, Model::PRELOAD_PER_POLICY);
    }
}

void TreeItemMessage::setPreviewFetchStatus(TreeItem::FetchingState state)
{
    m_previewFetchState = state;
}
...
QString TreeItemPart::partId() const
{
    ...
    } else if (dynamic_cast<TreeItemPart *>(parent())
               && dynamic_cast<TreeItemPart *>(parent())->mimeType() == "message/rfc822"
               && !mimeType().startsWith("multipart/")) {
        //if parent is message/rfc822, I am not a multipart, use parent part id
        QString parentId;
        TreeItemPart *parentPart = dynamic_cast<TreeItemPart *>(parent());
        return parentPart->partId() + ".TEXT";
    } else {
        QString parentId;
        TreeItemPart *parentPart = dynamic_cast<TreeItemPart *>(parent());
        Q_ASSERT(parentPart);
...
QString TreeItemPart::partIdForFetchPreview() const
{
    return QString::fromUtf8("BODY.PEEK[%1]<0.100>").arg(partId());
}
...
QVariant TreeItemPartMultipartMessage::data(Model * const model, int role)
{
    ...
    } else if (role == RoleMessageSubject) {
        fetch(model);
        return QVariant::fromValue(m_envelope.subject);
    } else if (role == RoleMessageDate) {
        fetch(model);
        return QVariant::fromValue(m_envelope.date);
    } else if (role == RoleMessageFrom) {
        fetch(model);
        return addresListToQVariant(m_envelope.from);
    } else {
...
void TreeItemPartMultipartMessage::setBodyText(TreeItemPart *partText)
{
    m_partText = partText;
}
