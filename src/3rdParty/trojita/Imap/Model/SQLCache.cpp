...
if ( ! q.exec( QLatin1String("CREATE TABLE msg_threading ( " \
                             ...
                             "threading BINARY" \
                             " )") ) ) { \
    emitError( QString("Can't create table msg_threading"), q ); \
    return false; \
}
...
if ( ! q.exec( QLatin1String("CREATE TABLE mailbox_sync_state ( " \
                             ...
                             "sync_state BINARY " \
                             " )") ) ) { \
    emitError( QString("Can't create table mailbox_sync_state"), q ); \
    return false; \
}
...
#define TROJITA_SQL_CACHE_CREATE_MSG_METADATA \
    if (! q.exec(QLatin1String("CREATE TABLE msg_metadata (" \
                               ...
                               ")"))) { \
        emitError(QString("Can't create table msg_metadata"), q); \
        return false; \
    }

#define TROJITA_SQL_CACHE_CREATE_MSG_PREVIEW \
    if (! q.exec(QLatin1String("CREATE TABLE msg_preview (" \
                               "mailbox STRING NOT NULL, " \
                               "uid INT NOT NULL, " \
                               "preview STRING, " \
                               "PRIMARY KEY (mailbox, uid)" \
                               ")"))) { \
        emitError(QString("Can't create table msg_preview"), q); \
        return false; \
    }
...
bool SQLCache::open(const QString &name, const QString &fileName)
{
    ...
    bool ok = db.open();
    if (! ok) {
        emitError(QString("Can't open database"), db);
        return false;
    }
    ...
    if (! q.exec(QLatin1String("SELECT version FROM trojita"))) {
        emitError(QString("Failed to verify version"), q);
        return false;
    }

    if (! q.first()) {
        // we could probably relax this...
        emitError(QString("Can't determine version info"), q);
        return false;
    }
    ...
    if (version == 1) {
        ...
        if (! q.exec(QLatin1String("UPDATE trojita SET version = 2;"))) {
            emitError(QString("Failed to update cache DB scheme from v1 to v2"), q);
            return false;
        }
    }
    ...
    if (version == 2 || version == 3) {
        ...
        if (!q.exec(QLatin1String("DROP TABLE mailbox_sync_state;"))) {
            emitError(QString("Failed to drop old table mailbox_sync_state"));
            return false;
        }
        TROJITA_SQL_CACHE_CREATE_SYNC_STATE;
        version = 4;
        if (! q.exec(QLatin1String("UPDATE trojita SET version = 4;"))) {
            emitError(QString("Failed to update cache DB scheme from v2/v3 to v4"), q);
            return false;
        }
    }
    ...
    if (version == 4 || version == 5) {
        ...
        if (!q.exec(QLatin1String("DROP TABLE msg_metadata;"))) {
            emitError(QString("Failed to drop old table msg_metadata"));
            return false;
        }
        TROJITA_SQL_CACHE_CREATE_MSG_METADATA;
        version = 6;
        if (! q.exec(QLatin1String("UPDATE trojita SET version = 6;"))) {
            emitError(QString("Failed to update cache DB scheme from v4/v5 to v6"), q);
            return false;
        }
    }

    if (version == 6) {
        if (! q.exec(QLatin1String("UPDATE trojita SET version = 7;"))) {
            emitError(QString("Failed to update cache DB scheme from v6 to v7"), q);
            return false;
        }
        TROJITA_SQL_CACHE_CREATE_MSG_PREVIEW;
        version = 7;
    }
    if (version != 7) {
        emitError(QString("Unknown version"));
        return false;
    }
...
bool SQLCache::createTables()
{
    ...
    if (! q.exec(QLatin1String("CREATE TABLE trojita ( version STRING NOT NULL )"))) {
        emitError(QString("Failed to prepare table structures"), q);
        return false;
    }
    if (! q.exec(QLatin1String("INSERT INTO trojita ( version ) VALUES ( 7 )"))) {
        emitError(QString("Can't store version info"), q);
        return false;
    }
    if (! q.exec(QLatin1String(
        ...
        emitError(QString("Can't create table child_mailboxes"));
        return false;
    }
    ...
    if (! q.exec(QLatin1String("CREATE TABLE uid_mapping ( "
                               ...
                               "mapping BINARY"
                               " )"))) {
        emitError(QString("Can't create table uid_mapping"), q);
        return false;
    }
    ...
    if (! q.exec(QLatin1String("CREATE TABLE flags ("
                               ...
                               "PRIMARY KEY (mailbox, uid)"
                               ")"))) {
        emitError(QString("Can't create table flags"), q);
    }
    ...
    if (! q.exec(QLatin1String("CREATE TABLE parts ("
                               ...
                               ")"))) {
        emitError(QString("Can't create table parts"), q);
    }
...
bool SQLCache::prepareQueries()
{
    ...
    if (! queryChildMailboxes.prepare("SELECT mailbox, separator, flags FROM child_mailboxes WHERE parent = ?")) {
        emitError(QString("Failed to prepare queryChildMailboxes"), queryChildMailboxes);
        return false;
    }
    ...
    if (! queryChildMailboxesFresh.prepare(QLatin1String("SELECT mailbox FROM child_mailboxes WHERE parent = ? LIMIT 1"))) {
        emitError(QString("Failed to prepare queryChildMailboxesFresh"), queryChildMailboxesFresh);
        return false;
    }
    ...
    if (!queryRemoveChildMailboxes.prepare(QLatin1String("DELETE FROM child_mailboxes WHERE parent = ?"))) {
        emitError(QString("Failed to prepare queryRemoveChildMailboxes"), queryRemoveChildMailboxes);
        return false;
    }
    ...
    if (! querySetChildMailboxes.prepare(QLatin1String("INSERT OR REPLACE INTO child_mailboxes ( mailbox, parent, separator, flags ) VALUES (?, ?, ?, ?)"))) {
        emitError(QString("Failed to prepare querySetChildMailboxes"), querySetChildMailboxes);
        return false;
    }
    ...
    if (! queryMailboxSyncState.prepare(QLatin1String("SELECT sync_state FROM mailbox_sync_state WHERE mailbox = ?"))) {
        emitError(QString("Failed to prepare queryMailboxSyncState"), queryMailboxSyncState);
        return false;
    }
    ...
    if (! querySetMailboxSyncState.prepare(QLatin1String("INSERT OR REPLACE INTO mailbox_sync_state "
                                           "( mailbox, sync_state ) "
                                           "VALUES ( ?, ? )"))) {
        emitError(QString("Failed to prepare querySetMailboxSyncState"), querySetMailboxSyncState);
        return false;
    }

    queryUidMapping = QSqlQuery(db);
    if (! queryUidMapping.prepare(QLatin1String("SELECT mapping FROM uid_mapping WHERE mailbox = ?"))) {
        emitError(QString("Failed to prepare queryUidMapping"), queryUidMapping);
        return false;
    }
    ...
    if (! querySetUidMapping.prepare(QLatin1String("INSERT OR REPLACE INTO uid_mapping (mailbox, mapping) VALUES  ( ?, ? )"))) {
        emitError(QString("Failed to prepare querySetUidMapping"), querySetUidMapping);
        return false;
    }

    queryClearUidMapping = QSqlQuery(db);
    if (! queryClearUidMapping.prepare(QLatin1String("DELETE FROM uid_mapping WHERE mailbox = ?"))) {
        emitError(QString("Failed to prepare queryClearUidMapping"), queryClearUidMapping);
        return false;
    }
    ...
    if (! queryMessageMetadata.prepare(QLatin1String("SELECT data, lastAccessDate FROM msg_metadata WHERE mailbox = ? AND uid = ?"))) {
        emitError(QString("Failed to prepare queryMessageMetadata"), queryMessageMetadata);
        return false;
    }
    ...
    if (!queryAccessMessageMetadata.prepare(QLatin1String("UPDATE msg_metadata SET lastAccessDate = ? WHERE mailbox = ? AND uid = ?"))) {
        emitError(QString("Failed to prepare queryAccssMessageMetadata"), queryAccessMessageMetadata);
        return false;
    }
    ...
    if (! querySetMessageMetadata.prepare(QLatin1String("INSERT OR REPLACE INTO msg_metadata ( mailbox, uid, data, lastAccessDate ) VALUES ( ?, ?, ?, ? )"))) {
        emitError(QString("Failed to prepare querySetMessageMetadata"), querySetMessageMetadata);
        return false;
    }

    queryMessageFlags = QSqlQuery(db);
    if (! queryMessageFlags.prepare(QLatin1String("SELECT flags FROM flags WHERE mailbox = ? AND uid = ?"))) {
        emitError(QString("Failed to prepare queryMessageFlags"), queryMessageFlags);
        return false;
    }
    ...
    if (! querySetMessageFlags.prepare(QLatin1String("INSERT OR REPLACE INTO flags ( mailbox, uid, flags ) VALUES ( ?, ?, ? )"))) {
        emitError(QString("Failed to prepare querySetMessageFlags"), querySetMessageFlags);
        return false;
    }
    ...
    if (! queryClearAllMessages1.prepare(QLatin1String("DELETE FROM msg_metadata WHERE mailbox = ?"))) {
        emitError(QString("Failed to prepare queryClearAllMessages1"), queryClearAllMessages1);
        return false;
    }

    queryClearAllMessages2 = QSqlQuery(db);
    if (! queryClearAllMessages2.prepare(QLatin1String("DELETE FROM flags WHERE mailbox = ?"))) {
        emitError(QString("Failed to prepare queryClearAllMessages2"), queryClearAllMessages2);
        return false;
    }

    queryClearAllMessages3 = QSqlQuery(db);
    if (! queryClearAllMessages3.prepare(QLatin1String("DELETE FROM parts WHERE mailbox = ?"))) {
        emitError(QString("Failed to prepare queryClearAllMessages3"), queryClearAllMessages3);
        return false;
    }
    ...
    if (! queryClearAllMessages4.prepare(QLatin1String("DELETE FROM msg_threading WHERE mailbox = ?"))) {
        emitError(QString("Failed to prepare queryClearAllMessages4"), queryClearAllMessages4);
        return false;
    }
    ...
    if (! queryMessagePart.prepare(QLatin1String("SELECT data FROM parts WHERE mailbox = ? AND uid = ? AND part_id = ?"))) {
        emitError(QString("Failed to prepare queryMessagePart"), queryMessagePart);
        return false;
    }
    ...
    if (! querySetMessagePart.prepare(QLatin1String("INSERT OR REPLACE INTO parts ( mailbox, uid, part_id, data ) VALUES (?, ?, ?, ?)"))) {
        emitError(QString("Failed to prepare querySetMessagePart"), querySetMessagePart);
        return false;
    }
    ...
    if (! queryForgetMessagePart.prepare(QLatin1String("DELETE FROM parts WHERE mailbox = ? AND uid = ? AND part_id = ?"))) {
        emitError(QString("Failed to prepare queryForgetMessagePart"), queryForgetMessagePart);
        return false;
    }
    ...
    if (! queryMessageThreading.prepare(QLatin1String("SELECT threading FROM msg_threading WHERE mailbox = ?"))) {
        emitError(QString("Failed to prepare queryMessageThreading"), queryMessageThreading);
        return false;
    }
    ...
    if (! querySetMessageThreading.prepare(QLatin1String("INSERT OR REPLACE INTO msg_threading (mailbox, threading) VALUES  ( ?, ? )"))) {
        emitError(QString("Failed to prepare querySetMessageThreading"), querySetMessageThreading);
        return false;
    }

    queryMessagePreview = QSqlQuery(db);
    if (! queryMessagePreview.prepare(QLatin1String("SELECT preview FROM msg_preview WHERE mailbox = ? AND uid = ?"))) {
        emitError(QString("Failed to prepare queryMessagePreview"), queryMessagePreview);
        return false;
    }

    querySetMessagePreview = QSqlQuery(db);
    if (! querySetMessagePreview.prepare(QLatin1String("INSERT OR REPLACE INTO msg_preview(mailbox, uid, preview) VALUES(?, ?, ?)"))) {
        emitError(QString("Failed to prepare querySetMessagePreview"), querySetMessagePreview);
        return false;
    }
...
QList<MailboxMetadata> SQLCache::childMailboxes(const QString &mailbox) const
{
    ...
    if (! queryChildMailboxes.exec()) {
        emitError(QString("Query queryChildMailboxes failed"), queryChildMailboxes);
        return res;
    }
    while (queryChildMailboxes.next()) {
        ...
        if (stream.status() != QDataStream::Ok) {
            emitError(QString("Corrupt data when reading child items for mailbox %1, line %2").arg(mailbox, item.mailbox));
...
bool SQLCache::childMailboxesFresh(const QString &mailbox) const
{
    ...
    if (! queryChildMailboxesFresh.exec()) {
        emitError(QString("Query queryChildMailboxesFresh failed"), queryChildMailboxesFresh);
        return false;
    }
...
void SQLCache::setChildMailboxes(const QString &mailbox, const QList<MailboxMetadata> &data)
{
    ...
    if (!queryRemoveChildMailboxes.exec()) {
        emitError(QString("Query queryRemoveChildMailboxes failed"), queryRemoveChildMailboxes);
        return;
    }
    ...
    if (! querySetChildMailboxes.execBatch()) {
        emitError(QString("Query querySetChildMailboxes failed"), querySetChildMailboxes);
        return;
    }
}
...
SyncState SQLCache::mailboxSyncState(const QString &mailbox) const
{
    ...
    if (! queryMailboxSyncState.exec()) {
        emitError(QString("Query queryMailboxSyncState failed"), queryMailboxSyncState);
        return res;
    }
...
void SQLCache::setMailboxSyncState(const QString &mailbox, const SyncState &state)
{
    ...
    if (! querySetMailboxSyncState.exec()) {
        emitError(QString("Query querySetMailboxSyncState failed"), querySetMailboxSyncState);
        return;
    }
}
...
QList<uint> SQLCache::uidMapping(const QString &mailbox) const
{
    ...
    if (! queryUidMapping.exec()) {
        emitError(QString("Query queryUidMapping failed"), queryUidMapping);
        return res;
    }
...
void SQLCache::setUidMapping(const QString &mailbox, const QList<uint> &seqToUid)
{
    ...
    if (! querySetUidMapping.exec()) {
        emitError(QString("Query querySetUidMapping failed"), querySetUidMapping);
    }
}
...
void SQLCache::clearUidMapping(const QString &mailbox)
{
    ...
    if (! queryClearUidMapping.exec()) {
        emitError(QString("Query queryClearUidMapping failed"), queryClearUidMapping);
    }
}
...
void SQLCache::clearAllMessages(const QString &mailbox)
{
    ...
    if (! queryClearAllMessages1.exec()) {
        emitError(QString("Query queryClearAllMessages1 failed"), queryClearAllMessages1);
    }
    if (! queryClearAllMessages2.exec()) {
        emitError(QString("Query queryClearAllMessages2 failed"), queryClearAllMessages2);
    }
    if (! queryClearAllMessages3.exec()) {
        emitError(QString("Query queryClearAllMessages3 failed"), queryClearAllMessages3);
    }
    if (! queryClearAllMessages4.exec()) {
        emitError(QString("Query queryClearAllMessages4 failed"), queryClearAllMessages4);
    }
...
void SQLCache::clearMessage(const QString mailbox, uint uid)
{
    ...
    if (! queryClearMessage1.exec()) {
        emitError(QString("Query queryClearMessage1 failed"), queryClearMessage1);
    }
    if (! queryClearMessage2.exec()) {
        emitError(QString("Query queryClearMessage2 failed"), queryClearMessage2);
    }
    if (! queryClearMessage3.exec()) {
        emitError(QString("Query queryClearMessage3 failed"), queryClearMessage3);
    }
}
...
QStringList SQLCache::msgFlags(const QString &mailbox, const uint uid) const
{
    ...
    if (! queryMessageFlags.exec()) {
        emitError(QString("Query queryMessageFlags failed"), queryMessageFlags);
        return res;
    }
...
void SQLCache::setMsgFlags(const QString &mailbox, const uint uid, const QStringList &flags)
{
    ...
    if (! querySetMessageFlags.exec()) {
        emitError(QString("Query querySetMessageFlags failed"), querySetMessageFlags);
    }
}
...
AbstractCache::MessageDataBundle SQLCache::messageMetadata(const QString &mailbox, uint uid) const
{
    ...
    if (! queryMessageMetadata.exec()) {
        emitError(QString("Query queryMessageMetadata failed"), queryMessageMetadata);
        return res;
    }
        ...
        if (m_updateAccessIfOlder) {
            ...
            if (lastAccessTimestamp < currentDiff - m_updateAccessIfOlder) {
                ...
                if (!queryAccessMessageMetadata.exec()) {
                    emitError(QString("Query queryAccessMessageMetadata failed"), queryAccessMessageMetadata);
                }
            }
        }
    }
...
void SQLCache::setMessageMetadata(const QString &mailbox, const uint uid, const MessageDataBundle &metadata)
{
    ...
    if (! querySetMessageMetadata.exec()) {
        emitError(QString("Query querySetMessageMetadata failed"), querySetMessageMetadata);
    }
}
...
QByteArray SQLCache::messagePart(const QString &mailbox, const uint uid, const QString &partId) const
{
    ...
    if (! queryMessagePart.exec()) {
        emitError(QString("Query queryMessagePart failed"), queryMessagePart);
        return res;
    }
...
void SQLCache::setMsgPart(const QString &mailbox, const uint uid, const QString &partId, const QByteArray &data)
{
    ...
    if (! querySetMessagePart.exec()) {
        emitError(QString("Query querySetMessagePart failed"), querySetMessagePart);
    }
}
...
void SQLCache::forgetMessagePart(const QString &mailbox, const uint uid, const QString &partId)
{
    ...
    if (! queryForgetMessagePart.exec()) {
        emitError(QString("Query queryForgetMessagePart failed"), queryForgetMessagePart);
    }
}
...
QVector<Imap::Responses::ThreadingNode> SQLCache::messageThreading(const QString &mailbox)
{
    ...
    if (! queryMessageThreading.exec()) {
        emitError(QString("Query queryMessageThreading failed"), queryMessageThreading);
        return res;
    }
...
void SQLCache::setMessageThreading(const QString &mailbox, const QVector<Imap::Responses::ThreadingNode> &threading)
{
    ...
    if (! querySetMessageThreading.exec()) {
        emitError(QString("Query querySetMessageThreading failed"), querySetMessageThreading);
    }

}

QString SQLCache::messagePreview(const QString mailbox, const uint uid)
{
    touchingDB();
    queryMessagePreview.bindValue(0, mailboxName(mailbox));
    queryMessagePreview.bindValue(1, uid);
    if (! queryMessagePreview.exec()) {
        emitError(QString("Query queryMessagePreview failed"), queryMessagePreview);
        return "";
    }
    if (queryMessagePreview.first()) {
        return queryMessagePreview.value(0).toString();
    }
    return "";
}

void SQLCache::setMessagePreview(const QString mailbox, const uint uid, const QString preview)
{
    touchingDB();
    querySetMessagePreview.bindValue(0, mailboxName(mailbox));
    querySetMessagePreview.bindValue(1, uid);
    querySetMessagePreview.bindValue(2, preview);
    if (! querySetMessagePreview.exec()) {
        emitError(QString("Query querySetMessagePreview failed"), querySetMessagePreview);
    }
}
