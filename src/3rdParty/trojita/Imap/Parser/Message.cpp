...
{
    Mailbox::TreeItemChildrenList list;
    Mailbox::TreeItemPartMultipartMessage *part = new Mailbox::TreeItemPartMultipartMessage(parent, envelope);
    Mailbox::TreeItemChildrenList childList = body->createTreeItems(part);
    part->setChildren(childList);     // always returns an empty list -> no need to qDeleteAll()
    if (!dynamic_cast<MultiMessage *>(body.data())) {
        //setBodyText so that it works when refer child using <partid>.TEXT
        part->setBodyText(dynamic_cast<Mailbox::TreeItemPart *>(childList[0]));
    }
