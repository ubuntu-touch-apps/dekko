...
Fetch::Fetch(const uint number, const QByteArray &line, int &start): number(number)
{
    ...
    while (start < line.size() && line[start] != ')') {
        ...
        // Add "<0>" to the identifier. "<0>" would appear if this is a fetching preview
        // operation. This would change identifier from "BODY[0.1]" to "BODY[0.1]<0>"
        if (line.indexOf("<0>", start) == start) {
            identifier.append("<0>");
            start = start + 3;
        }
