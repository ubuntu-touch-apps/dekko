...
#define PRINT_TRAFFIC 500
#define PRINT_TRAFFIC_TX 500
#define PRINT_TRAFFIC_RX 100
...
CommandHandle Parser::authenticate(const QByteArray &saslMethod, const QString &accessToken)
{
    return queueCommand(Commands::Command("AUTHENTICATE") <<
                        Commands::PartOfCommand(saslMethod) << Commands::PartOfCommand(accessToken.toUtf8()));
}
...
void Parser::handleReadyRead()
{
    while (!waitingForEncryption && !waitingForSslPolicy) {
        ...
        case ReadingNumberOfBytes:
        {
            ...
            currentLine += buf;
            if (fileDownloadProgressSignalChoke.isValid()) {
                if (fileDownloadProgressSignalChoke.hasExpired(fileDownloadProgressSignalInterval)) {
                    emit fileDownloadProgress(currentLine.size(), fileDownloadingTotalSize);
                    fileDownloadProgressSignalChoke.restart();
                }
            } else {
                fileDownloadProgressSignalChoke.start();
            }
            if (readingBytes == 0) {
...
void Parser::reallyReadLine()
{
    try {
        ...
        if (currentLine.endsWith("}\r\n")) {
            ...
            readingBytes = number;
            fileDownloadingTotalSize = currentLine.mid(offset + 1, currentLine.lastIndexOf('}') - offset - 1).toInt();
