    ...
    shouldRunNoop(false), shouldRunIdle(false), idleLauncher(0), unSelectTask(0)
{
    ...
    fetchPreviewTimer = new QTimer(this);
    connect(fetchPreviewTimer, SIGNAL(timeout()), this, SLOT(slotFetchPreviews()));
    fetchPreviewTimer->setInterval(0);
    fetchPreviewTimer->setSingleShot(true);
...
void KeepMailboxOpenTask::requestPreviewDownload(const uint uid, const QString &partId)
{
    requestedPreviews[partId].append(uid);
    if (!fetchPreviewTimer->isActive()) {
        fetchPreviewTimer->start();
    }
}
...
void KeepMailboxOpenTask::slotFetchPreviews() {
    if (requestedPreviews.isEmpty())
        return;
    breakOrCancelPossibleIdle();
    QMap<QString, QList<uint> >::iterator it = requestedPreviews.begin();
    while (it != requestedPreviews.end()
            && (shouldExit || fetchPartTasks.size() < limitParallelFetchTasks))
    {
        QList<uint> uids = *it;
        QList<uint>::iterator uidIt = uids.begin();
        QList<uint> outputUids;
        uint totalSize = 0;
        while (uidIt != uids.end()
              && (outputUids.size() < limitMessagesAtOnce && totalSize < limitBytesAtOnce))
        {
            outputUids << *uidIt;
            totalSize += 100;
            uidIt = uids.erase(uidIt);
        }
        fetchPartTasks << model->m_taskFactory->createFetchMsgPartTask(model, mailboxIndex, outputUids, QStringList() << it.key());
        if (uidIt == uids.end()) {
            it = requestedPreviews.erase(it);
        }
    }
}
