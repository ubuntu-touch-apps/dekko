    ...
    ImapTask(model)
{
    ...
    connect(parser, SIGNAL(fileDownloadProgress(qint64,qint64)), model, SIGNAL(fileDownloadProgress(qint64, qint64)));
        ...
        // Check the result of the LOGIN command
    {
        if (resp->tag == loginCmd) {
            ...
            if (resp->kind == OK) {
                ...
                emit model->authAttemptSucceeded();
            } else {
                // Login failed
                QString message;
...
{
    if (!model->oauthAccessToken().isEmpty() && model->accessParser(parser).capabilities.contains(QLatin1String("AUTH=XOAUTH2"))) {
        Q_ASSERT(authenticateCmd.isEmpty());
        authenticateCmd = parser->authenticate("XOAUTH2", model->oauthAccessToken());
        model->accessParser(parser).capabilitiesFresh = false;
    } else if (model->m_hasImapPassword) {
