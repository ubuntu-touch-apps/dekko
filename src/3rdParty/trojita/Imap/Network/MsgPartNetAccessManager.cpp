...
QNetworkReply *MsgPartNetAccessManager::createRequest(Operation op, const QNetworkRequest &req, QIODevice *outgoingData)
{
    ...
    if (!message.isValid()) {
        ...
        // There is an exception here that if this is a gravatar request we let it through
        if (req.url().host() != QLatin1String("www.gravatar.com") && req.url().host() != QLatin1String("cdn.libravatar.org")) {
            return new Imap::Network::ForbiddenReply(this);
        }
    }
    if (req.url().host() == QLatin1String("www.gravatar.com") || req.url().host() == QLatin1String("cdn.libravatar.org")) {
        // We save all gravatar requests in the gravatar cache. Since this QNAM is created per request
        // and only lives for the life time of the request we can safely reset the cache on every gravatar
        // request and set PreferCache to improve reload times.
//        QNetworkDiskCache* cache = new QNetworkDiskCache(this);
//        cache->setCacheDirectory(QString("%1/gravatar").arg(QStandardPaths::writableLocation(QStandardPaths::CacheLocation)));
//        this->setCache(cache);
        // Copy the request as we need to add the PreferCache attribute
    ...
    if (req.url().scheme() == QLatin1String("trojita-imap") && req.url().host() == QLatin1String("msg")) {
        ...
        if (partIndex.isValid()) {
            return new Imap::Network::MsgPartNetworkReply(this, partIndex, req.url().hasQuery());
        } else {
