...
void FileDownloadManager::downloadPart()
{
    if (!partIndex.isValid()) {
        emit transferError(QString("FileDownloadManager::downloadPart(): part has disappeared"));
        return;
    }
...
{
    if (!partIndex.isValid()) {
        emit transferError(QString("FileDownloadManager::downloadMessage(): message has disappeared"));
        return;
    }
    QString saveFileName = toRealFileName(partIndex);
    Q_ASSERT(!m_combiner);
