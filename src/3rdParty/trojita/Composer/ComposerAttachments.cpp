...
int FileAttachmentItem::sizeInBytes()
{
...
int ImapMessageAttachmentItem::sizeInBytes()
{
    return index.data(Imap::Mailbox::RoleMessageSize).toInt();
}
...
QString ImapPartAttachmentItem::tooltip() const
{
    if (!index.isValid())
        return QString();
    return QString("%1, %2").arg(index.data(RolePartMimeType).toString(),
...
int ImapPartAttachmentItem::sizeInBytes()
{
    return index.data(RolePartOctets).toInt();
}
