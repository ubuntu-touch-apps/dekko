...
void ProcessSocket::handleProcessError(QProcess::ProcessError err)
{
    ...
    emit disconnected(QString("The QProcess is having troubles: %1").arg(proc->errorString()));
}
...
void ProcessSocket::handleStateChanged()
{
    ...
    case QProcess::Running:
        emit stateChanged(Imap::CONN_STATE_CONNECTED_PRETLS_PRECAPS, QString("The process has started"));
        break;
    case QProcess::Starting:
        emit stateChanged(Imap::CONN_STATE_CONNECTING, QString("Starting process `%1 %2`").arg(executable, args.join(QLatin1String(" "))));
        break;
    case QProcess::NotRunning: {
        ...
        if (stdErr.isEmpty())
            disconnectedMessage = QString("The QProcess has exited with return code %1.").arg(
                                      proc->exitCode());
        else
            disconnectedMessage = QString("The QProcess has exited with return code %1:\n\n%2").arg(
...
void SslTlsSocket::close()
{
    ...
    Q_ASSERT(sock);
    sock->abort();
    emit disconnected(QString("Connection closed"));
}
...
void SslTlsSocket::handleStateChanged()
{
    ...
    case QNetworkProxy::Socks5Proxy:
        proxyMsg = QString(" (via SOCKS5 proxy %1)").arg(sock->proxy().hostName());
        break;
    case QNetworkProxy::HttpProxy:
        proxyMsg = QString(" (via HTTP proxy %1)").arg(sock->proxy().hostName());
        break;
    }
    switch (sock->state()) {
    case QAbstractSocket::HostLookupState:
        emit stateChanged(Imap::CONN_STATE_HOST_LOOKUP, QString("Looking up %1%2...").arg(host,
                              sock->proxy().capabilities().testFlag(QNetworkProxy::HostNameLookupCapability) ?
                              proxyMsg : QString()));
        break;
    case QAbstractSocket::ConnectingState:
        emit stateChanged(Imap::CONN_STATE_CONNECTING, QString("Connecting to %1:%2%3%4...").arg(
                              host, QString::number(port), startEncrypted ? QString(" (SSL)") : QString(),
    ...
    case QAbstractSocket::ConnectedState:
        if (! startEncrypted) {
            emit stateChanged(Imap::CONN_STATE_CONNECTED_PRETLS_PRECAPS, QString("Connected"));
        } else {
            emit stateChanged(Imap::CONN_STATE_SSL_HANDSHAKE, QString("Negotiating encryption..."));
        }
        break;
    case QAbstractSocket::UnconnectedState:
    case QAbstractSocket::ClosingState:
        disconnectedMessage = QString("Socket is disconnected: %1").arg(sock->errorString());
        delayedDisconnect->start();
        break;
    }
}
...
void SslTlsSocket::handleSocketError(QAbstractSocket::SocketError err)
{
    ...
    Q_ASSERT(sock);
    delayedDisconnect->stop();
    emit disconnected(QString("The underlying socket is having troubles when processing connection to %1:%2: %3").arg(
...
void SslTlsSocket::delayedStart()
{
    ...
    case Streams::ProxySettings::RespectSystemProxy:
    {
        ...
        if (!settingFound || proxySettingsList.isEmpty()) {
            ...
            emit disconnected(QString("The underlying socket is having troubles when processing connection to %1:%2: %3")
